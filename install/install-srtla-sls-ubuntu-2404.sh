...
#   * Klont den gepflegten Fork  github.com/Bittersweet1987/srtla  (srtla2)
#     statt des alten Marlow925/srtla (srtla v1)
...
#   1. STATS_KEY aus der Umgebung -> nutzen.
#   2. Interaktives Terminal      -> fragen (Auto / eigener / aus).
#   3. Kein Terminal (Pipe)       -> automatisch Key generieren.
...
echo "Downloading and installing SRT Server (v1.5.3). This can take a few minutes."
cd "$USER_HOME"
if [ ! -d srt ]; then
...
echo "Installing SRTLA Relay Server (gepflegter Fork Bittersweet1987/srtla, srtla2)"
cd "$USER_HOME"
# NEU: gepflegten Fork (srtla2) klonen. Ein evtl. vorhandenes altes 'srtla'
# (Marlow925 = srtla v1, oder BELABOX) wird auf den Fork umgestellt.
if [ ! -d srtla ]; then
